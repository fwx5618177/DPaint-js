/**
 * Application command identifiers. Triggered through the {@link EventBus} to
 * request an action (menu items, toolbar buttons, keyboard shortcuts ...).
 *
 * Values are kept identical to the original DPaint.js implementation so that
 * serialized references (e.g. menu configuration, host messages) remain valid.
 */
...
/**
 * Internal application events. Triggered through the {@link EventBus} to notify
 * subscribers that some state has changed.
 */
export const EVENT = {
  drawColorChanged: 1,
...
export type EventValue = (typeof EVENT)[EventKey];

/** Built-in animation kinds used by the animator/recorder. */
...
export type AnimationValue = (typeof ANIMATION)[AnimationKey];

/** Any identifier accepted by the {@link EventBus}. */
export type BusTopic = CommandValue | EventValue | string | number;
